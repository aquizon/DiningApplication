class MenuItemsController < ApplicationController
  ...
  def get_allergens
    allergens = ''
    diets = %w[dairy gluten soy nuts]
    diets.each do |diet|
      allergens += "#{diet.capitalize}, " unless params[diet].nil?
    end
    allergens = allergens.slice(0..-3) if allergens != ''
    allergens
  end

  def get_diets
    diets = ''
    groups = %w[vegan vegetarian]
    groups.each do |item|
      diets += "#{item.capitalize}, " unless params[item].nil?
    end
    diets = diets.slice(0..-3) if diets != ''
    diets
  end
